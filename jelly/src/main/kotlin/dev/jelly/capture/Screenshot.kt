...
object Screenshot {
    ...
    /**
     * Captures the entire Activity window via PixelCopy (or `View.draw` on a
     * decor-fallback). The resulting bitmap is in **window** coordinates so
     * any captured-element bounds expressed via `SemanticsCapture.captureInWindow`
     * line up with bitmap pixels 1:1 — that's what makes the
     * Application-level install (`Jelly.install`) work uniformly across
     * apps that span multiple Compose roots, native Views, dialogs, etc.
     */
    ...
    private suspend fun captureRegion(
        ...
        val outDir = File(context.cacheDir, "jelly").apply { mkdirs() }
    ...
    private suspend fun pixelCopy(window: Window, src: Rect, dest: Bitmap): Boolean =
        suspendCoroutine { cont ->
            val thread = HandlerThread("jelly-pixelcopy").apply { start() }
