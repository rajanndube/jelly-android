package dev.agentation.sync

import dev.agentation.model.Annotation
import dev.agentation.model.Session
import dev.agentation.model.SessionWithAnnotations
 ...
 * is serialized with the same field names. Failures throw [AgentationApiException]
...
class AgentationApi(
    ...
    private fun HttpResponse.requireOk(): HttpResponse {
        if (!status.isSuccess()) {
            throw AgentationApiException(status.value, "HTTP ${status.value} ${status.description}")
        }
        return this
    }
...
class AgentationApiException(val statusCode: Int, message: String) : RuntimeException(message)
